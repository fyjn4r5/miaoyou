...
const HeaderMailbox: React.FC<HeaderMailboxProps> = ({ 
  ...
  // feat: 统一使用全局通知
  const { showSuccessMessage, showErrorMessage, mailbox: currentMailbox, isPasswordVisible, togglePasswordVisibility } = useContext(MailboxContext);
  ...
  const loginButtonClass = `${buttonBaseClass} hover:bg-primary/20 hover:text-primary hover:scale-110 mr-1`;
  ...
  const copyPasswordToClipboard = () => {
    if (currentMailbox?.password) {
      navigator.clipboard.writeText(currentMailbox.password)
        .then(() => {
          showSuccessMessage(t('mailbox.copyPasswordSuccess'));
        })
        .catch(() => {
          showErrorMessage(t('mailbox.copyPasswordFailed'));
        });
    }
  };
          ...
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex items-center">
              ...
              {currentMailbox?.password && (
                <div className="flex items-center ml-2">
                  <button
                    ...
                    className="flex items-center px-2 py-1 text-xs rounded-md bg-muted hover:bg-muted/80 transition-colors"
                    title={showPassword ? t('mailbox.hidePassword') : t('mailbox.showPassword')}
                  >
                    <i className={`fas ${showPassword ? 'fa-eye-slash' : 'fa-eye'} text-xs mr-1`}></i>
                    {showPassword ? t('mailbox.hidePassword') : t('mailbox.showPassword')}
                  </button>
                  {showPassword && (
                    <button
                      onClick={copyPasswordToClipboard}
                      className="ml-1 w-7 h-7 flex items-center justify-center rounded-md hover:bg-primary/20 hover:text-primary transition-colors"
                      title={t('common.copy')}
                    >
              ...
              <button
                ...
                className={`w-8 h-8 ${loginButtonClass}`}
                title={t('mailbox.login')}
              >
