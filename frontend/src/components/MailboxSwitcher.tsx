...
const MailboxSwitcher: React.FC<MailboxSwitcherProps> = ({
  ...
  const [savedMailboxes, setSavedMailboxes] = useState<(Mailbox & { password: string })[]>([]);
  ...
  useEffect(() => {
    if (currentMailbox) {
      updateSavedMailboxes(currentMailbox as Mailbox & { password: string });
    }
  }, [currentMailbox]);
  ...
  const loadSavedMailboxes = () => {
    try {
      ...
      if (savedData) {
        const mailboxes = JSON.parse(savedData) as (Mailbox & { password: string })[];
  ...
  // 更新保存的邮箱列表
  const updateSavedMailboxes = (mailbox: Mailbox & { password: string }) => {
    try {
      // 获取当前保存的邮箱
      let mailboxes: (Mailbox & { password: string })[] = [];
      try {
        const savedData = localStorage.getItem('savedMailboxes');
        if (savedData) {
          mailboxes = JSON.parse(savedData) as (Mailbox & { password: string })[];
        }
      } catch (error) {
  ...
  // 删除单个已保存的邮箱
  const handleDeleteMailbox = async (address: string, password?: string) => {
    if (window.confirm(t('mailbox.confirmDeleteMailbox'))) {
      // 调用API删除后端的邮箱
      const result = await apiDeleteMailbox(address, password);
      if (result.success) {
  ...
  const handleClearAllMailboxes = async () => {
    if (window.confirm(t('mailbox.confirmClearAllMailboxes'))) {
      ...
      // 并行执行所有后端删除请求
      const deletePromises = mailboxesToDelete.map(m => apiDeleteMailbox(m.address, m.password));
            ...
            {savedMailboxes.map((m) => (
              <div key={m.address} className="flex items-center justify-between hover:bg-muted rounded-sm">
                ...
                {(!currentMailbox || m.address !== currentMailbox.address) && (
                  <button
                    onClick={() => handleDeleteMailbox(m.address, m.password)}
